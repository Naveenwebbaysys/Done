...
//
//   let getReelsResponseModel = try? JSONDecoder().decode(GetReelsResponseModel.self, from: jsonData)

import Foundation

// MARK: - GetReelsResponseModel
struct GetReelsResponseModel: Codable {
    let status: Bool?
    let data: DataClass?
}
...
struct Post: Codable {
    ...
    let tags: [JSONAny]?
    let addLinks: [String]?
    let videoURL,projectType: String?
