...
var APIModel = ServiceController()
...
class ServiceController: NSObject {
    ...
    func optionsRequest(strURL:String,postHeaders:NSDictionary,success:@escaping(_ result:Any)->Void,failure:@escaping(_ error:String) -> Void) {
        if isConnectedToNetwork() == false {
            print("Please Check Internet")
...
//        let urlComp = NSURLComponents(string: "https://www.drrecommendations.com/api/posts/comment.php?" + "task_created_by={task_created_by}&employee_id={employee_id}&assignee_employee_id={assignee_employee_id}")!
    ...
    func patchRequest(strURL:NSString,postParams:Encodable,postHeaders:NSDictionary,successHandler:@escaping( _ result:Any)->Void,failureHandler:@escaping (_ error:String)->Void) -> Void {
        if isConnectedToNetwork() == false {
            print("Please Check Internet")
            return
        }
...
//            KRProgressHUD.dismiss()
    ...
    func deleteRequest(strURL:NSString,postParams:Encodable?,postHeaders:NSDictionary,successHandler:@escaping( _ result:Any)->Void,failureHandler:@escaping (_ error:String)->Void) -> Void {
        if isConnectedToNetwork() == false {
            print("Please Check Internet")
            return
        }
